...
namespace VTEventSchedule.Models
{
    public class clsCode
    {
        ...
        {
            query = string.Format("exec CAMDBSh.VM_Proc_GetQuickGuideByContainer '{0}'", containerName);
            return db.GetDataRecord(query);
        }

        public DataView GetHoldTargetContainer(string factoryName)
        {
            query = string.Format("exec CAMDBsh.VM_Proc_GetHoldTargetContainer N'{0}'", factoryName);
            return db.GetDataView("Hold", query);
        }

        public DataView GetReleaseTargetContainer(string factoryName, string releaseReasonName)
        {
            query = string.Format("exec CAMDBsh.VM_Proc_GetReleaseTargetContainer N'{0}', N'{1}'", factoryName, releaseReasonName);
            return db.GetDataView("Release", query);
        }

        public DataView GetHoldTargetEmployee(string workType,  string factoryName)
        {
            query = string.Format("exec CAMDBsh.VM_Proc_GetHoldTargetEmployee '{0}', N'{1}'", workType, factoryName);
            return db.GetDataView("Hold", query);
        }
        public DataView GetReleaseTargetEmployee(string workType, string factoryName, string holdReasonName)
        {
            query = string.Format("exec CAMDBsh.VM_Proc_GetReleaseTargetEmployee '{0}', N'{1}', N'{2}'", workType, factoryName, holdReasonName);
            return db.GetDataView("Release", query);
        }
    }
}
