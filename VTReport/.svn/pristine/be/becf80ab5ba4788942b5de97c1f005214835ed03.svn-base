...
public partial class View_Dashboard_DashboardViewAlert : System.Web.UI.Page
{
        ...
        if (doc.Element("Dashboard").Element("Parameters") != null)
        {
            ...
            if (parameters.Count() > 0)
            {
                foreach (XElement param in parameters)
                {
                    ...
                    if (param.Attribute("Name").Value == "__userName")
                    {
                        param.Attribute("Value").Value = Session["EmpNo"].ToString();
                    }
