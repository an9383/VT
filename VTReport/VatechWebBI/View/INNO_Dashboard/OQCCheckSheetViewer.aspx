<%@ Page Language="C#" AutoEventWireup="true" CodeFile="OQCCheckSheetViewer.aspx.cs" Inherits="View_INNO_Dashboard_OQCCheckSheetViewer" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        function OpenPop_OQC() {
            const arrKey = ['ContainerId', 'Gubun', 'AttributeValue','StepEntryTxnId'];

            var urlParam = "&WorkFlowStepName=OQC";
            ...
            $.each(axisPoint.GetDimensions(), function (_, dimension) {
                ...
                if (found != undefined) {
                    ...
                    //레포트 AttributeValue별 URL 변경 할때
                    if (found == 'AttributeValue') {
                        Model = axisPoint.GetDimensionValue(dimension.Id).GetValue();
                    }
                }
            });
            var url;
            //var url = "../ReportView.aspx?reportid=17" + urlParam;
            //alert(Model);
            
            if ( Model == 'ProVecta HD') {
                url = "../ReportView.aspx?reportid=17" + urlParam;
            }
            else if (Model == 'VEX-P250(R5)') {
                url = "../ReportView.aspx?reportid=18" + urlParam;
            }
            else if (Model == 'VEX-P250(R6)') {
                url = "../ReportView.aspx?reportid=21" + urlParam;
            }
            else if (Model == 'VEX-P300(R5)') {
                url = "../ReportView.aspx?reportid=22" + urlParam;
            }
            else if (Model == 'VEX-P300(R6)') {
                url = "../ReportView.aspx?reportid=23" + urlParam;
            }
            else if (Model == 'VEX-S100W') {
                url = "../ReportView.aspx?reportid=24" + urlParam;
            }
            else if (Model == 'VEX-S300C') {
                url = "../ReportView.aspx?reportid=25" + urlParam;
            }
            else if (Model == 'VEX-S300M') {
                url = "../ReportView.aspx?reportid=26" + urlParam;
            }
            else if (Model == 'VEX-S300W') {
                url = "../ReportView.aspx?reportid=27" + urlParam;
            }
            else if (Model == 'VistaIntra DC') {
                url = "../ReportView.aspx?reportid=28" + urlParam;
            }
