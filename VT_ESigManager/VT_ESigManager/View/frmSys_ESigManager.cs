...
namespace VT_ESigManager.View
{
    public partial class frmSys_ESigManager : DevExpress.XtraEditors.XtraForm
    {
        ...
        {
            //if (WrGlobal.EmployeeId == "")
            //{
            //    if (!OpenDialog_ChangeUser())
            //    {
            //        this.Close();
            //        return;
            //    }
        ...
        private void gvESigHistory_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            switch (drv["WorkflowStepName"].ToString().Substring(0, 3))
            {
                ...
                case "OQC":
                    reportUrl = String.Format(@"{0}/ReportServer/Pages/ReportViewer.aspx?/System/Certification/OQC/ReportViewer/OQC_Viewer&rc:showbackbutton=true&id={1}&ws=OQC&steptxnid={2}&pConID={1}",
                                            WrGlobal.reportRootUrl, drv["ContainerName"].ToString(), drv["StepEntryTxnId"].ToString());
                    break;

                default:
                    reportUrl = String.Format(@"{0}/ReportServer/Pages/ReportViewer.aspx?/System/History/CheckSheet/ReportViewer/CheckSheetViewer&rc:showbackbutton=true&id={1}&ws=DD&steptxnid={2}",
                                            WrGlobal.reportRootUrl, drv["ContainerName"].ToString(), drv["StepEntryTxnId"].ToString());
                    break;
            }
